...
typedef struct pdf_font_s
{
  char      *pFontName;
  unsigned long flags;
  long  llx; long  lly; long  urx; long  ury; //bbox
  float  italicAngle;
  int    ascender;
  int    descender;
  //leading
  ...
  //missingWidth
    ...
    long    lGdixs[256];
    long    uchar[256];
    std::map<long, long> *m_pmapUnicodeGidx;
    std::map<long, long> *m_pmapUnicodeWidth;
    size_t filelen;
    unsigned char *img;
    long    cff_offset;
    size_t    cff_length;
