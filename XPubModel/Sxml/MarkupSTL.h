...
class XPUBMODEL_EXPORTIMPORT CMarkupSTL // MIK
{
public:
	...
	//[CMARKUPDEV
	bool SetDoc( const std::string& strDoc );
	bool FindPrevElem( const char* szName=NULL );
	bool FindPrevChildElem( const char* szName=NULL );
	std::string FindGetData( const char* szName );
	int GetElemIndex() const { return m_iPos; };
	void GotoElemIndex( int i ) { x_SetPos(m_aPos[i].iElemParent,i,0); };
	int GetChildElemIndex() const { return m_iPosChild; };
	void GotoChildElemIndex( int i ) { x_SetPos(m_aPos[m_aPos[i].iElemParent].iElemParent,m_aPos[i].iElemParent,i); };
	bool GetOffsets( int* pnStart, int* pnLen, int* pnInStart=NULL, int* pnInLen=NULL ) const;
	bool GetAttribOffsets( const char* szAttrib, int* pnStart, int* pnLen, int* pnInStart=NULL, int* pnInLen=NULL ) const;
	std::string GetElemPath() const { return x_GetPath(m_iPos); };
	std::string GetChildElemPath() const { return x_GetPath(m_iPosChild); };
	int GetElemFlags() const { return m_aPos[m_iPos].nFlags & MNF_PUBLIC; };
	//]CMARKUPDEV
	std::string GetAttrib( const char* szAttrib ) const { return x_GetAttrib(m_iPos,szAttrib); };
	long GetAttribLong( const char* szAttrib ) const; // MIK
	...
	bool FindSetData( const char* szName, const char* szData, int nFlags=0 );
	//]CMARKUPDEV
	...
	//[CMARKUPDEV
	static std::string GetDeclaredEncoding( const char* szDoc );
	...
	static std::string UTF8ToA( const char* pszUTF8 );
	static std::string AToUTF8( const char* pszANSI );
	//]CMARKUPDEV
...
protected:
	...
	struct XPUBMODEL_EXPORTIMPORT SavedPosMap // MIK
	{
	...
	struct XPUBMODEL_EXPORTIMPORT PosArray // MIK
	{
		PosArray() { Clear(); };
		~PosArray() { Release(); };
	...
	int x_GetFreePos()
	{
		...
		if ( m_iPosDeleted )
		{
			...
			m_iPosDeleted = m_aPos[iPos].iElemNext;
			return iPos;
		}
		//]CMARKUPDEV
	...
	//[CMARKUPDEV
	int x_FindPrevElem( int iPosParent, int iPos, const char* szName );
