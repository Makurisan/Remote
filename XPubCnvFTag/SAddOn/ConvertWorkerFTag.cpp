...
// Grafik
  ...
  { TAG_IMAGE,           tCASE_GRAFIC_BAUSTEIN,  CConvertWorkerFromTag::Image },
	...
	{ TAG_INLINE,           tCASE_NONE,  CConvertWorkerFromTag::Inline },
	{ TAG_IMPORT,           tCASE_NONE,  CConvertWorkerFromTag::Import },
...
// Dokument
  ...
  { TAG_MODULE,			 tCASE_NONE, CConvertWorkerFromTag::Module },
  { TAG_ASSET,			 tCASE_NONE, CConvertWorkerFromTag::Asset },
  { TAG_PAGE,				 tCASE_NONE, CConvertWorkerFromTag::Page },
  { TAG_SPREAD,      tCASE_NONE, CConvertWorkerFromTag::Spread },
...
// Rectangle
  { TAG_RECTANGLE,				tCASE_NONE,	 CConvertWorkerFromTag::Rectangle },
...
bool CConvertWorkerFromTag::GetLibraryStream(const xtstring& sLibraryName, xtstring& sDataStream)
{
	map<xtstring, xtstring>::iterator cIt; 
	cIt = m_TagLibraryList.find(sLibraryName);
  if (cIt != m_TagLibraryList.end()) // Variable ist vorhanden
	{	
		sDataStream = ((*cIt).second).c_str();
		return true;
	}
	return false;
}
...
bool CConvertWorkerFromTag::Inline(CConvertWorkerFromTag* pCW, const xtstring& cInTag, void* lpToken)
{ 
  CHECKCALLPOINTER()
  ...
  xtstring sGraficFileName;
  if ( cInTag.FindNoCase(TAG_INLINE_FILENAME) != xtstring::npos)
  {
    GETPARAM1(TAG_INLINE_FILENAME, cParValue[0]);
    sGraficFileName = cParValue[0];
  }
  double dWidth = -1;
  if ( cInTag.FindNoCase(TAG_INLINE_WIDTH) != xtstring::npos)
  {
    GETPARAM1(TAG_INLINE_WIDTH, cParValue[0]);
    dWidth = atof(cParValue[0].c_str());
  }

	double dHeight = -1;
  if ( cInTag.FindNoCase(TAG_INLINE_HEIGHT) != xtstring::npos)
  {
    GETPARAM1(TAG_INLINE_HEIGHT, cParValue[0]);
    dHeight = atof(cParValue[0].c_str());
  }

// Anpassung der Grafik Indesign
	int nFitting = 0; // interpreted from width, height
  if ( cInTag.FindNoCase(TAG_IMAGE_FITTING) != xtstring::npos)
  {
	  GETPARAM1(TAG_IMAGE_FITTING, cParValue[0]);
		if (cParValue[0].FindNoCase(TAG_IMAGE_FIT_CONTPROP) != xtstring::npos)
			nFitting = TAG_IMAGE_FITCONTPROP;
		else
		if (cParValue[0].FindNoCase(TAG_IMAGE_FIT_FILLCONTPROP) != xtstring::npos)
			nFitting = TAG_IMAGE_FITFILLCONTPROP;
		else
		if (cParValue[0].FindNoCase(TAG_IMAGE_FIT_FRAMETOCONTENT) != xtstring::npos)
			nFitting = TAG_IMAGE_FITFRAMETOCONTENT;
		else
		if (cParValue[0].FindNoCase(TAG_IMAGE_FIT_CENTERCONTENT) != xtstring::npos)
			nFitting = TAG_IMAGE_FITCENTERCONTENT;
		else
		if (cParValue[0].FindNoCase(TAG_IMAGE_FIT_CONTENTTOFRAME) != xtstring::npos)
			nFitting = TAG_IMAGE_FITCONTENTTOFRAME;
	}

	xtstring sName(_XT("")); // a symbolic name of the grafic
  if ( cInTag.FindNoCase(TAG_INLINE_NAME) != xtstring::npos)
  {
	  GETPARAM1(TAG_INLINE_NAME, cParValue[0]);
		sName = cParValue[0];
	}

	xtstring sID(_XT("")); // a symbolic name of the grafic
  if ( cInTag.FindNoCase(TAG_INLINE_ID) != xtstring::npos)
  {
	  GETPARAM1(TAG_INLINE_ID, cParValue[0]);
		sID = cParValue[0];
	}

	double dXScale(0.0); // X-Scale des Bildes
  if ( cInTag.FindNoCase(TAG_INLINE_XSCALE) != xtstring::npos)
  {
	  GETPARAM1(TAG_INLINE_XSCALE, cParValue[0]);
    dXScale = atof(cParValue[0].c_str());
	}

	double dYScale(0.0); // Y-Scale des Bildes
  if ( cInTag.FindNoCase(TAG_INLINE_YSCALE) != xtstring::npos)
  {
	  GETPARAM1(TAG_INLINE_XSCALE, cParValue[0]);
    dYScale = atof(cParValue[0].c_str());
	}

	xtstring sNative(_XT("")); // a symbolic name of the grafic
  if ( cInTag.FindNoCase(TAG_INLINE_NATIVE) != xtstring::npos)
  {
	  GETPARAM1(TAG_INLINE_NATIVE, cParValue[0]);
		sNative = cParValue[0];
	}

	int nType = 0; // a symbolic name of the grafic
  if ( cInTag.FindNoCase(TAG_INLINE_TYPE) != xtstring::npos)
  {
	  GETPARAM1(TAG_INLINE_TYPE, cParValue[0]);
		nType = 0;
	}

	pCW->m_pConvertTo->Inline(sID, sName, sGraficFileName, dWidth, dHeight, sNative, dXScale, dYScale, nType, nFitting);
  return true;
}

bool CConvertWorkerFromTag::Import(CConvertWorkerFromTag* pCW, const xtstring& cInTag, void* lpToken)
{ 
  CHECKCALLPOINTER()
...
// Anpassung der Grafik Indesign
  ...
  if ( cInTag.FindNoCase(TAG_IMAGE_FITTING) != xtstring::npos)
  {
	  GETPARAM1(TAG_IMAGE_FITTING, cParValue[0]);
		...
		if (cParValue[0].FindNoCase(TAG_IMAGE_FIT_FRAMERIGHTTOCONTENT) != xtstring::npos)
			nFitting = TAG_IMAGE_FITFRAMERIGHTTOCONTENT;
		else
		if (cParValue[0].FindNoCase(TAG_IMAGE_FIT_FRAMETOCONTENT) != xtstring::npos)
			nFitting = TAG_IMAGE_FITFRAMETOCONTENT;
		else
  ...
  if ( cInTag.FindNoCase(TAG_INLINE_TYPE) != xtstring::npos)
  {
	...
	pCW->m_pConvertTo->Import(sID, sName, sGraficFileName, dWidth, dHeight, sNative, nType, nFitting);
  return true;
}
  ...
  if (lpLitToken->nCase == tCASE_GRAFIC_NORMAL)
  {
    ...
    if (cImageName.size() > 0)
    {
      ...
      if(pCW->IsPathFile(cTempPath.c_str()))   
        pCW->m_pConvertTo->InsertImage(xtstring(_XT("")), xtstring(_XT("")), cTempPath, -1, -1, 100, true, false);
      
      return true;
    }
  } 
  // Grafikbaustein ausgeben
  else if (lpLitToken->nCase == tCASE_GRAFIC_BAUSTEIN)
  {
		bool bIndCS = pCW->m_pConvertTo->GetFormaterType() == INDESIGN_Formater;

    xtstring cParValue[4];
    xtstring sPath;

		xtstring sName(_XT("")); // a symbolic name of the grafic
    if ( cInTag.FindNoCase(TAG_IMAGE_SYMNAME) != xtstring::npos)
    {
	    GETPARAM1(TAG_IMAGE_SYMNAME, cParValue[0]);
			sName = cParValue[0];
		}

    xtstring cGraficName;
    if ( cInTag.FindNoCase(TAG_IMAGE_FILENAME) != xtstring::npos)
    {
      // parameter freeware kontrolle
      if (pCW->m_pKeywords && pCW->m_bOnlyFreewareKeywordsUsed)
        pCW->CheckParameter(sKeyword, TAG_IMAGE_FILENAME);
      GETPARAM1(TAG_IMAGE_FILENAME, cParValue[0]);
      cGraficName = cParValue[0];
    }
	// compatibility with old versions esp. RTF, PDF
		if(!bIndCS && cGraficName.length() <= 0)
			cGraficName = sName;

    sPath = cGraficName;
  // check if image file exist but not with Indesign
    if( !pCW->IsPathFile(sPath.c_str()) && !bIndCS)
    {
    ...
    if ( cInTag.FindNoCase(TAG_IMAGE_WIDTH) != xtstring::npos)
    {
      ...
      dWidth = pCW->ConvertUnitOfLength(cParValue[0].c_str());
    }
    ...
    if ( cInTag.FindNoCase(TAG_IMAGE_HEIGHT) != xtstring::npos)
    {
      ...
      dHeight = pCW->ConvertUnitOfLength(cParValue[0].c_str());
    }
...
// Anpassung der Grafik Indesign
    ...
    if ( cInTag.FindNoCase(TAG_IMAGE_FITTING) != xtstring::npos)
    {
		...
		xtstring sID(_XT("")); // a symbolic name of the grafic
    if ( cInTag.FindNoCase(TAG_IMAGE_ID) != xtstring::npos)
    {
	    GETPARAM1(TAG_IMAGE_ID, cParValue[0]);
			sID = cParValue[0];
		}
    ...
    if((bShrink || bThumbnail) && pCW->GetTemporaryFileName(sPath, cTempPath, sType) )
    {
      ...
      if(bThumbnail == false)
      {
        if(nZoom != -1)
          ...
          cPicture.Resize(sPath, cTempPath, dWidth, dHeight, nDPI, nDPI);
      }
      else
      {
...
// Ist Grafik vorhanden oder Indesign?
    if(pCW->IsPathFile(cTempPath.c_str()) || sName.size() || bIndCS)   
    {
      pCW->m_pConvertTo->InsertImage(sID, sName, cTempPath, dWidth, dHeight, nZoom, true, bShrink, nFitting);
    }
    return true;
  }
  return false;
...
// #define GET_ROW(x)   (x == 0xffff)?m_nTableRow:x
  ...
  { 
		int locRow = pCW->m_pConvertTo->GetTableCol() ;
		if(locRow == 0)
			wCol = 2;
		else
			wCol = locRow + 1;
...
// Zelle aktivieren
 ...
 // eine Tabellenzeile einfügen
  ...
  if (cInTag.FindNoCase(TAG_TABLE_ADDLINE) != xtstring::npos)
  {
    ...
    {
			int nRelative=TAG_TABLE_ROW_APPEND_END;
			int nRowCount = 1;
			if(cInTag.FindNoCase(TAG_TABLE_ADDLINE_MORE) != xtstring::npos)
			{
				GETPARAM2(TAG_TABLE_ADDLINE_MORE, cParValue[0], cParValue[1]);
				nRowCount = max(1, atol(cParValue[0].c_str()));
			
				if(cParValue[1].FindNoCase(_XT("Before")) != xtstring::npos)
					nRelative=TAG_TABLE_ROW_APPEND_CURRENT;
				else
					nRelative=TAG_TABLE_ROW_APPEND_END;

			}

      if (pCW->m_pConvertTo->InsertTableRow(pCW->m_pConvertTo->GetTableID(), nRelative, nRowCount))
      {
...
// Zeile als Kopf definieren, wird bei Seitenumbruch wiederholt
    ...
    {  
      ...
      GETPARAM2(TAG_TABLE_HEADERROW, cParValue[0], cParValue[1]);
      
	    int nFromRow = atof(cParValue[0].c_str());
		  int nToRow = atof(cParValue[1].c_str());
			if(nToRow == 0)
				pCW->m_pConvertTo->SetTableHeaderRow(pCW->m_pConvertTo->GetTableID(), nRow, nCol, (nFromRow != 0));
			else
				pCW->m_pConvertTo->SetTableHeaderRow(pCW->m_pConvertTo->GetTableID(), nFromRow, nToRow, 2);
    }
    else
...
// Zeile mit weiteren Zeilen zusammenhalten
 ...
 // Name der Tabelle für Indesign zum Befüllen
	xtstring sName = _XT("");
	if (cInTag.FindNoCase(TAG_TABLE_NAME) != xtstring::npos)
  {
    GETPARAM1(TAG_TABLE_NAME, cParValue[0]);
    sName = cParValue[0];
  }
 // ElementID
	xtstring sID = _XT("");
	if (cInTag.FindNoCase(TAG_TABLE_ID) != xtstring::npos)
  {
    GETPARAM1(TAG_TABLE_ID, cParValue[0]);
    sID = cParValue[0];
  }

 // Table Clear="text"
 // dieser Befehl entfernt den kompletten Text aus einer Tabelle
 //	wird in Indesign für die Schwarzfilmverarbeitung benötigt
	if (cInTag.FindNoCase(TAG_TABLE_CLEAR) != xtstring::npos)
  {
    GETPARAM1(TAG_TABLE_CLEAR, cParValue[0]);
    int nModus = atol(cParValue[0].c_str());
		return pCW->m_pConvertTo->SetTableClear(sID, sName, nModus);
  }

// Space before and after the table
	double dSpaceBefore = 0l, dSpaceAfter = 0l;
	if (cInTag.FindNoCase(TAG_TABLE_SPACE) != xtstring::npos)
  {
    GETPARAM2(TAG_TABLE_SPACE, cParValue[0], cParValue[1]);
    dSpaceBefore = atof(cParValue[0].c_str());
    dSpaceAfter = atof(cParValue[1].c_str());
  }
  ...
  {    
   // Absatz auf Standard zurückstellen
    pCW->m_pConvertTo->SetParagraphIndents(0,0,0);
	 // Das Problem hier ist, dass in Indesign die Tabelle nun Left im Textrahmen steht	  
    ...
    int nRowCnt = max(atoi(cParValue[0].c_str()),1);
    int nColCnt = max(atoi(cParValue[1].c_str()),1);
		int numHeaderRows = atoi(cParValue[2].c_str());
		int numFooterRows = atoi(cParValue[3].c_str());

    int nTableID = pCW->m_pConvertTo->CreateTable(sID, sName, nRowCnt, nColCnt, 
			numHeaderRows, numFooterRows, dSpaceBefore, dSpaceAfter);
...
// minimale Tabellenzeilenhöhe festlegen
  if (cInTag.FindNoCase(TAG_TABLE_ROW_HEIGHT) != xtstring::npos)
  {
    ...
    GETPARAM2(TAG_TABLE_ROW_HEIGHT, cParValue[0], cParValue[1]);
    
    double fRowHeight=0;
		fRowHeight = max(1, atof(cParValue[0].c_str()));

		long nExactOrAtLeast = -1;

		if (cParValue[1].FindNoCase(TAG_TABLE_ROW_HEIGHT_EXACTLY) != xtstring::npos)
			nExactOrAtLeast = 1;
		else
		if (cParValue[1].FindNoCase(TAG_TABLE_ROW_HEIGHT_ATLEAST) != xtstring::npos)
			nExactOrAtLeast = 0;
		else
		{
			if(!cParValue[1].empty() && cParValue[1] != _XT("-1"))
				nExactOrAtLeast = cParValue[1] == _XT("1")?1:0;
		}

    pCW->m_pConvertTo->SetRowHeight(pCW->m_pConvertTo->GetTableID(),
                                           nRow, nCol, fRowHeight, nExactOrAtLeast);
  }
  ...
  if (cInTag.FindNoCase(TAG_TABLE_CELL_WIDTH) != xtstring::npos)
  {
    ...
    if ( cParValue[0].size() == 0 || cParValue[0] == _XT("-1") )
      fWidth = -1;
			...
			fWidth = max(1, atof(cParValue[0].c_str()));
...
// Tabellenzelle mit Rahmen belegen
  if (cInTag.FindNoCase(TAG_TABLE_BORDER_WIDTH) != xtstring::npos)
  {
	...
	// In Indesign wird in Points bei Zellbreite gerechnet
		if(pCW->m_pConvertTo->GetFormaterType() == INDESIGN_Formater)
		{
			nLeft = (int)(atof(cParValue[0].c_str()) * 100);
			nTop = (int)(atof(cParValue[1].c_str()) * 100);
			nRight = (int)(atof(cParValue[2].c_str()) * 100);
			nButton = (int)(atof(cParValue[3].c_str()) * 100);
		}
...
// Tabellenzelle aktivieren und löschen
  ...
  if (cInTag.FindNoCase(TAG_TABLE_SPLITCOLUMN) != xtstring::npos)
  {
	...
	if (cInTag.FindNoCase(TAG_TABLE_MERGECOLUMNS) != xtstring::npos)
  {
    GETPARAM4(TAG_TABLE_MERGECOLUMNS, cParValue[0], cParValue[1], cParValue[2], cParValue[3]);

    int nFromRow = max(atoi(cParValue[0].c_str()),1);
    int nFromCol = max(atoi(cParValue[1].c_str()),1);
    int nToRow = max(atoi(cParValue[2].c_str()),1);
    int nToCol = max(atoi(cParValue[3].c_str()),1);

    pCW->m_pConvertTo->MergeColumns(pCW->m_pConvertTo->GetTableID(), nFromRow, nFromCol, nToRow, nToCol);
  }
...
bool CConvertWorkerFromTag::Checkbox(CConvertWorkerFromTag* pCW, const xtstring& cInTag, void* lpToken)
{
  ...
  if ( cInTag.FindNoCase(TAG_CHECKBOX_VALUE) != xtstring::npos)
  {
    ...
    {
      return pCW->m_pConvertTo->InsertImage(xtstring(_XT("")), xtstring(_XT("")), sPath, -1, -1, 100, true, false);
    }
    else
    {
...
bool CConvertWorkerFromTag::Box(CConvertWorkerFromTag* pCW, const xtstring& cInTag, void* lpToken)
{
  ...
  if (lpLitToken->nCase == tCASE_BOX)
  {
    ...
    int nTableID = pCW->m_pConvertTo->CreateTable(xtstring(_XT("")), xtstring(_XT("")), 1, 1, 0, 0, 0l, 0l);
    pCW->m_pConvertTo->SetTableID(nTableID);
    pCW->m_pConvertTo->SetTableRow(1);
    pCW->m_pConvertTo->SetTableCol(1);
...
bool CConvertWorkerFromTag::Rectangle(CConvertWorkerFromTag* pCW, const xtstring& cInTag, void* lpToken)
{
  CHECKCALLPOINTER()
...
#define TAG_RECTANGLE_WIDTH				"width="				// Breite des Bildes
#define TAG_RECTANGLE_HEIGHT			"height="				// Höhe des Bildes
...
*/
  ...
  // ID der Textbox
  xtstring sName;
  if (cInTag.FindNoCase(TAG_RECTANGLE_SYMNAME) != xtstring::npos)
  {
    // parameter freeware kontrolle
    GETPARAM1(TAG_RECTANGLE_SYMNAME, cParValue[0]);
    sName = cParValue[0];
  }
  // Name der Textbox
  xtstring sId;
  if (cInTag.FindNoCase(TAG_RECTANGLE_ID) != xtstring::npos)
  {
    // parameter freeware kontrolle
    GETPARAM1(TAG_RECTANGLE_ID, cParValue[0]);
    sId = cParValue[0];
  }
	...
	xtstring LineColor; 
  if (cInTag.FindNoCase(TAG_RECTANGLE_BORDERCOLOR) != xtstring::npos)
  {
    // parameter freeware kontrolle
    GETPARAM1(TAG_RECTANGLE_BORDERCOLOR, cParValue[0]);
    LineColor = cParValue[0];
  }

	xtstring FillColor;
  if (cInTag.FindNoCase(TAG_RECTANGLE_FILLCOLOR) != xtstring::npos)
  {
    // parameter freeware kontrolle
    GETPARAM1(TAG_RECTANGLE_FILLCOLOR, cParValue[0]);
    FillColor = cParValue[0];
  }
  return pCW->m_pConvertTo->CreateRectangle(sId, sName, nLeft, nTop, nWidth, nHeight,
...
*/
	...
	xtstring sLibrary;
  if (cInTag.FindNoCase(TAG_TEXTFRAME_LIBRARY) != xtstring::npos)
  {
    GETPARAM1(TAG_TEXTFRAME_LIBRARY, cParValue[0]);
		{
			sLibrary = cParValue[0];
		}
	}
...
// MAK: muss evtl. noch in PDF/RTF als Standard verlagert werden
  ...
  int nTxtFrameID;
  bool bReturn = 0 != (nTxtFrameID = pCW->m_pConvertTo->CreateTextFrame(xtstring(_XT("")), m_sTxtFrame, sJoin, sLibrary,
...
bool CConvertWorkerFromTag::Spread(CConvertWorkerFromTag* pCW, const xtstring& cInTag, void* lpToken)
{
  CHECKCALLPOINTER()
  ...
  if (pCW->m_nWorkModus != tWM_Work)
	...
	int nFrom, nTo, nType, nCount;

	nFrom=nTo=nType=nCount=0;

	return pCW->m_pConvertTo->Spread(nFrom, nTo, nType, nCount);

}

bool CConvertWorkerFromTag::Module(CConvertWorkerFromTag* pCW, const xtstring& cInTag, void* lpToken)
{
  CHECKCALLPOINTER()
  ...
  if (pCW->m_nWorkModus != tWM_Work)
	...
	xtstring sLayer(_XT("")); 
  if ( cInTag.FindNoCase(TAG_MODULE_LAYER) != xtstring::npos)
  {
	  GETPARAM1(TAG_MODULE_LAYER, cParValue[0]);
		sLayer = cParValue[0];
	}

	xtstring sFunction(_XT("")); 
  if ( cInTag.FindNoCase(TAG_MODULE_FUNCTION) != xtstring::npos)
  {
	  GETPARAM1(TAG_MODULE_FUNCTION, cParValue[0]);
		sFunction = cParValue[0];
	}

	xtstring sID(_XT("")); // a symbolic name
  if ( cInTag.FindNoCase(TAG_MODULE_ID) != xtstring::npos)
  {
	  GETPARAM1(TAG_MODULE_ID, cParValue[0]);
		sID = cParValue[0];
	}

  xtstring sParameter;
  if ( cInTag.FindNoCase(TAG_MODULE_PARAMETER) != xtstring::npos)
  {
    GETPARAM1(TAG_MODULE_PARAMETER, cParValue[0]);
    sParameter = cParValue[0];
  }

	return pCW->m_pConvertTo->Module(sID, sFunction, sParameter, sLayer);
}


bool CConvertWorkerFromTag::Asset(CConvertWorkerFromTag* pCW, const xtstring& cInTag, void* lpToken)
{
  CHECKCALLPOINTER()
  ...
  if (pCW->m_nWorkModus != tWM_Work)
	...
	xtstring sName(_XT("")); // the asset name
  if ( cInTag.FindNoCase(TAG_ASSET_NAME) != xtstring::npos)
  {
	  GETPARAM1(TAG_ASSET_NAME, cParValue[0]);
		sName = cParValue[0];
	}

	xtstring sID(_XT("")); // a symbolic name
  if ( cInTag.FindNoCase(TAG_ASSET_ID) != xtstring::npos)
  {
	  GETPARAM1(TAG_ASSET_ID, cParValue[0]);
		sID = cParValue[0];
	}

  xtstring sFileName;
  if ( cInTag.FindNoCase(TAG_ASSET_FILENAME) != xtstring::npos)
  {
    GETPARAM1(TAG_ASSET_FILENAME, cParValue[0]);
    sFileName = cParValue[0];
  }

	return pCW->m_pConvertTo->Asset(sID, sName, sFileName);
}

bool CConvertWorkerFromTag::Page(CConvertWorkerFromTag* pCW, const xtstring& cInTag, void* lpToken)
{
  CHECKCALLPOINTER()
  ...
  if (pCW->m_nWorkModus != tWM_Work)
	...
	xtstring sLib;
	if ( cInTag.FindNoCase(TAG_PAGE_LIB) != xtstring::npos)
  {
    GETPARAM1(TAG_PAGE_LIB, cParValue[0]);
    sLib = cParValue[0];
  }

	xtstring sRepeat;
	if ( cInTag.FindNoCase(TAG_PAGE_REPEAT) != xtstring::npos)
  {
    GETPARAM1(TAG_PAGE_REPEAT, cParValue[0]);
    sRepeat = cParValue[0];
  }
	
	xtstring sOverset; 
	if ( cInTag.FindNoCase(TAG_PAGE_OVERFLOW) != xtstring::npos)
  {
    GETPARAM1(TAG_PAGE_OVERFLOW, cParValue[0]);
    sOverset = cParValue[0];
  }

	xtstring sJoin;  
	if ( cInTag.FindNoCase(TAG_PAGE_JOIN) != xtstring::npos)
  {
    GETPARAM1(TAG_PAGE_JOIN, cParValue[0]);
    sJoin = cParValue[0];
  }

	xtstring sAsset;  
	if ( cInTag.FindNoCase(TAG_PAGE_ASSET) != xtstring::npos)
  {
    GETPARAM(TAG_PAGE_ASSET, cParValue[0]);
    sAsset = cParValue[0];
  }

	return pCW->m_pConvertTo->Page(sJoin, sAsset, sLib, sOverset, sRepeat, nFrom, nTo, nType);
...
bool CConvertWorkerFromTag::FormField(CConvertWorkerFromTag* pCW, const xtstring& cInTag, void* lpToken)
{
  ...
  if ( cInTag.FindNoCase(TAG_FORMFIELD_NAME) != xtstring::npos)
  {
   ...
   // spezielle Tabellenverarbeitung
...
bool CConvertWorkerFromTag::Layer_CS(CConvertWorkerFromTag* pCW, const xtstring& cInTag, void* lpToken)
{
  CHECKCALLPOINTER()
  ...
  xtstring cParValue[4];
	...
	int nDelete=-1; 
	if ( cInTag.FindNoCase(TAG_LAYER_CS_DELETE) != xtstring::npos)
	{
		GETPARAM(TAG_LAYER_CS_DELETE, cParValue[0]);
		nDelete = atol(cParValue[0].c_str());
	}

	int nShow=-1; 
	if ( cInTag.FindNoCase(TAG_LAYER_CS_SHOW) != xtstring::npos)
	{
		GETPARAM(TAG_LAYER_CS_SHOW, cParValue[0]);
		nShow = atol(cParValue[0].c_str()) == 0?0:1;
	}

	int nGuide=0; 
	if ( cInTag.FindNoCase(TAG_LAYER_CS_GUIDE) != xtstring::npos)
	{
		GETPARAM(TAG_LAYER_CS_GUIDE, cParValue[0]);
		nShow = atol(cParValue[0].c_str()) == 0?0:1;
	}

	int nLock=0; 
	if ( cInTag.FindNoCase(TAG_LAYER_CS_LOCK) != xtstring::npos)
	{
		GETPARAM(TAG_LAYER_CS_LOCK, cParValue[0]);
		nLock = atol(cParValue[0].c_str()) == 0?0:1;
	}

	int nActive=-1; 
	if ( cInTag.FindNoCase(TAG_LAYER_CS_ACTIVATE) != xtstring::npos)
	{
		GETPARAM(TAG_LAYER_CS_ACTIVATE, cParValue[0]);
		nActive = atol(cParValue[0].c_str()) == 0?0:1;
	}
	...
	{
		GETPARAM(TAG_FONT_COLOR, cParValue[0]);
		sSwatch = atol(cParValue[0].c_str()) == 0?0:1;
	}

	xtstring sCpyFrom; 
	if ( cInTag.FindNoCase(TAG_LAYER_CS_COPYFROM) != xtstring::npos)
	{
		GETPARAM(TAG_LAYER_CS_COPYFROM, cParValue[0]);
		sCpyFrom = cParValue[0].c_str();
	}

	int nMoveTo=-2; // Layer in Liste an angegebene Position verschieben, -1=Ende, 0=Anfang
	if ( cInTag.FindNoCase(TAG_LAYER_CS_MOVETO) != xtstring::npos)
	{
		GETPARAM(TAG_LAYER_CS_MOVETO, cParValue[0]);
		nMoveTo = atol(cParValue[0].c_str());
	}

	return pCW->m_pConvertTo->Layer_CS(sName, sSwatch, sCpyFrom, nShow, nGuide, nLock, nActive, nDelete, nMoveTo);
...
/////////////////////// CS2 /////////////////////////////////////////////
	...
	if(pCW->m_pConvertTo->GetFormaterType() == INDESIGN_Formater)
		return Layer_CS(pCW, cInTag, lpToken);
...
double CConvertWorkerFromTag::ConvertUnitOfLength(LPCXTCHAR lpValue)
{
	//char *str = setlocale(LC_NUMERIC, "English");
	//::MessageBox(0,str,"",0 );
...
double CConvertWorkerFromTag::ConvertUnitOfLength(double nValue)
{
  double nConvert = 0;
...
double CConvertWorkerFromTag::MilimeterToTwips(double nValue)
{
...
double CConvertWorkerFromTag::InchToTwips(double nValue)
{
  // ins milimeter
  //nValue *= 25.4;
  //return ((double) ((((double)nValue) * 1440 * 10) / (double)254));
	double nRet = 0;
	nRet =  nValue * 1440;
	return nRet;
}

double CConvertWorkerFromTag::PicaToTwips(double nValue)
{
  long nRet = 0;

  if (nValue <= ( LONG_MAX / 240 ) && nValue >= ( LONG_MIN / 240 ) )
    nRet = (double)(nValue * (double)240);
  return nRet;
}
